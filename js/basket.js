...
/****************DELETE ONE PRODUCT (icone trash)*************/
...
// Test email valid
    if(mail.value.match(emailRegExp)){
        msgMail.textContent="Votre email est valide"; 
...
/************** validity INPUT (NAME- FIRSTNAME-CITY)***********/
function checkCivil() {
    if(myName.value.match(civilId) 
    && myFirstName.value.match(civilId) 
    && myTown.value.match(civilId)){
    console.log("correct**********$")
    buttonForm.style.visibility="visible"; 
    }else{
    console.log("---------NOP-------")
    //alert("Vérifier le format de vos champs!");
...
function checkStreet(){
    ...
    console.log("correct   STREET **********$")
  }else{
   console.log("---------NOP  STREET -------")
    //alert("Vérifier le format de vos champs!");
  }
